                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12">
                            <div class="form-group">
                                <textarea class="editor summernote" name="content"></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12">
                            ...
                            <img src="" id="image" class="rounded mx-auto d-block w-100 logo mt-2 mb-2">
                        </div>
                    </div>
                </form>
            </div>
...
<div id="myFeedback" class="modal fade " role="dialog">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="formFeedback" action="<?= base_url() ?>Feedback/addFeedback/<?= $id_ormawa ?>" method="post" class="needs-validation" novalidate="" onsubmit={this.saveAndContinue}>
                    <div class="form-group row">
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <a href="#" id="pubicon" class="btn btn-primary m-2">Publish Article</a>
                    </div>
                </div>
            </div>
            <div class="card">
...
<script>
    ...
    $(function() {
        ...
        $('#show_data').on('click', '.item-detail', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    refresh();
                    $('#pubicon').attr('href', '<?= base_url() ?>article/publishArticle/' + id + "/<?= $id_ormawa ?>");
                    if (data.status_article == 2) {
                        $('#pubicon').hide();
                    }else{
                        $('#pubicon').show();
                    }
        ...
        $('#show_data').on('click', '.item-feedback', function() {
            $("#myFeedback").trigger('reset');
            $('#myFeedback').modal('show');
            $('#myFeedback').find('.modal-title').text('Feedback');
            $('#myForm').attr('action', '<?php echo base_url() ?>Feedback/addFeedback');
            var id = $(this).attr('data');
        ...
        $('#show_data').on('click', '.item-edit', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#kontenarea').html(data.content_article);
    ...
    function showAllArticle() {
        ...
        $.ajax({
            url: '<?php echo base_url() ?>Article/showArticleByOrmawa/<?= $id_ormawa ?>',
            async: false,
            dataType: 'json',
            success: function(data) {
                // console.log(data);
                ...
                for (i = 0; i < data.length; i++) {
                    ...
                    html += '<tr>' +
                        ...
                        '<td class="text-center"><span class="badge badge-pill badge-' + color + '">' + text + '</span></td>' +
                        ...
                        '<button class="btn btn-info item-feedback mr-2" data="' + data[i].id_article + '"><i class="fas fa-marker" aria-hidden="true"></i></button>' +
                        '</td>' +
                        '</tr>';
                }
                MyTable.fnDestroy();
