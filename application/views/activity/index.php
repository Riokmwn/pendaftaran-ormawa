...
<script>
        ...
        $('#show_data').on('click', '.item-edit', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('input[name=nama_dokumen]').val(data.document_activity);
                },
                error: function() {
                    alert('Error!');
                }
    ...
    function showAllActivity() {
        ...
        if (role == 1 || role == 2) {
            show_url = 'showAllActivity';
        } else {
            show_url = 'showActivityByOrmawa/<?= $ormawa_id ?>'
        }

        $.ajax({
            url: '<?php echo base_url() ?>Activity/' + show_url,
            async: false,
            dataType: 'json',
            success: function(data) {
                // console.log(data);
                ...
                for (i = 0; i < data.length; i++) {
                    ...
                    html += '<tr>' +
                        ...
                        <?php if ($user['role_id'] == 1 || $user['role_id'] == 2) { ?> '<td class="">' + data[i].name_ormawa + '</td>' +
                        <?php } ?> '<td class=""><a href="<?= base_url('assets/document/lpj/') ?>' + data[i].document_activity + '" type="button" class="btn btn-success">Download</a></td>' +
                        ...
                        '<button class="btn btn-warning item-edit mr-2" data="' + data[i].id_activity + '"><i class="fa fa-edit" aria-hidden="true"></i></button>' +
                        '<button class="btn btn-danger item-delete" data="' + data[i].id_activity + '"><i class="fa fa-trash" aria-hidden="true"></i></button>' +
                        <?php if ($user['role_id'] == 1 || $user['role_id'] == 2) { ?>
                        '<button class="btn btn-info item-terima mt-2" data="' + data[i].id_activity + '">Terima</button>' +
...
<?php
if ($user['role_id'] == 4 || $user['role_id'] == 5 || $user['role_id'] == 2) { ?>
    <script>
        $(document).ready(function() {
            $('.item-edit').remove();
            $('.item-delete').remove();
